...
for year in range(1910, 2000, 10):
    for i in range(0, len(OccupationD)):
        if year == OccupationD[i][0]:
            Occupation_difference.append(100 * OccupationD[i][5])  # Gain All the percentages of occupation in selected year
    #print(year)
...
#Difference = []
#for i in range(len(perc_90_20)):
    #Difference.append(np.log(perc_90_20[i] / (1 - perc_90_20[i])))
...
def Cal(year, year_vec):
    ...
    sns.regplot(x = difference, y = Bias_1990)
